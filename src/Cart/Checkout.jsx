...
const Checkout = (props) => {
  ...
  return (
    !canProceed
      ? <Box>
        {hour
      ? <GreenButton onClick={(_) => props.onTryCheckout({ total })}>Proceed to checkout
        </GreenButton>
...
const mapStateToProps = (store) => ({
  ...
  total: store.cart.addedIds.map((v, i) => v.cost * store.cart.quantityById[v.productId]).reduce((a, v) => a + v, 0),
